...
"""
Calculate number of words and letters from previous Homeworks 5/6 outputs test file.
Create two csv:
...
def count(array, item):
    counter = 0
    for i in array:
        counter += 1 if i == item else counter
    return counter
...
def count_words_csv_writer():
    ...
    with open("outputs/countWords.csv", "w", newline ="") as csvfile:
...
def count_letters_csv_writer():
    ...
    with open("outputs/countLetters.csv", "w", newline ="") as csvfile:
        writer = csv.DictWriter(csvfile, fieldnames = fieldnames)
